...
Every subcommand below accepts the same six [global flags](/commands/global-flags/),
...
is [the complete command reference](/commands/every-command/) — generated from the
...
<CardGrid>
  <LinkCard
    ...
    description="The six flags every subcommand takes, and the ones that only look global."
  />
  <LinkCard
  ...
  <LinkCard
    ...
    description="Generated from what the binary parses, at the revision this site pins."
  />
  <LinkCard
    title="Words we use"
...
| `update`  | Moves something onto a newer version — `stack` or `self`                 | You want the services on the versions this build pins, or want to know where this copy of lemonfiber stands |
...
| `held`       | Shows what one member can actually watch, read from the media server as them | You want to see what an age limit or a library restriction comes to, as the list it produces rather than the setting |
...
| `history` | Lists every change lemonfiber made, and how far each could be put back | You want to know what changed, when, and whether it can be undone                                                                                         |
| `undo`    | Puts back one run of changes, named by the stamp `history` shows       | You want a seed or a reconfigure reversed. The whole run or none of it — the history says how many changes go with each entry                             |
...
| `wiring` | Says what the stack wires to what, and how each link was settled; `wiring fill` chooses which service fills a capability | Two services could answer the same need, or something asks for a capability nothing provides. The choice is journalled, and `undo` puts it back |
...
| Command       | What it does                                                                         | Reach for it when                                                                                                                                               |
| ------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `catalogue`   | Says what each service in this stack is for, and what became of any it dropped       | The service names convey nothing on their own and you want a sentence for each: what it does for you, what you lose while it is down, and how much that costs   |
| `provenance`  | Says where each service comes from: its licence, its project, and the version pinned | You want to check that what is bundled here is open source rather than take it on faith. It reads the stack description, so it answers with the containers down |
...
| Command  | What it does                                                          | Reach for it when                                                                                                                                                                                                                                                       |
| -------- | --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `plugin` | Installs, updates and removes plugins, and reads what one may declare | You are adding a service the stack does not bundle, or writing one. `install`, `update` and `remove` change the machine and take `--dry-run`; `installed` lists what is here; `capabilities`, `extension-points`, `schema`, `claims` and `provenance` are for an author |

`install` and `update` take a path — the plugin's directory, or the `plugin.toml`
inside it. Neither is kept unless the plugin's own proofs hold against its running
service and the stack's checks still pass afterwards; if either fails, the install
is put back. There is no disable: a plugin is installed or it is not, and `remove`
takes its id. The author's reads need no network, no catalogue and no running
stack, and each names the generation it reports — except `provenance`, the one
read that asks a registry about an image's signature.
[Plugins](/plugins/) is the whole of it.
